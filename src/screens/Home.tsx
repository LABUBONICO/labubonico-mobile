...
const Home = ({ navigation }: NativeStackScreenProps<MainStackParamList>) => {
  ...
  const fetchReceipts = async () => {
    try {
      ...
      const docs = snapshot.docs.map((doc) => ({ ...doc.data() })) as Receipt[];
      console.log(docs);
    ...
    <View style={styles.container}>
      {
        isLoading ?
          ...
          <View style={{ ...styles.container }}>
            <View style={{ alignSelf: "flex-start", marginTop: 40, borderBottomWidth: 2, borderBottomColor: "black", width: "90%", marginInline: 20}}>
              <Text>Suas despesas do mês</Text>
              <Text style={{ fontSize: 48, fontWeight: "bold" }}>R$ {balance.toFixed(2)}</Text>
            </View>
            {
              receiptsList.length === 0 ?
                ...
                <FlatList
                  ...
                  renderItem={({ item }) =>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", marginBlock: 10, padding: 10, minWidth: "90vw" }}>
